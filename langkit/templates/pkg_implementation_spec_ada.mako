...
private package ${ada_lib_name}.Implementation is
   ...
   type Analysis_Context_Type is limited record
      ...
      --  rebindings validity checking when for nodes: once we have established
      --  that the node reference is valid regarding its context, we know that
      --  the rebindings pointer is valid, and thus we can just check the
      --  rebinding version number.
   end record;
